...
pub fn get_data(day: &str) -> Result<(String, String), miette::ErrReport> {
    ...
    if &day[0..11] != "src/bin/aoc" { return Err(miette!("Bad day name."))}
    ...
    if path.exists() {
        return std::fs::read_to_string(path).map(|body|(body,filename)).into_diagnostic();
    }
    
    dotenv().ok();

    let contents = get(&format!("https://adventofcode.com/2022/day/{day}/input"))
