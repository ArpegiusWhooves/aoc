...
fn part1( numbers: Vec<i32> ) -> Result<i64> {
    let numbers_len = numbers.len() as i64;

    let mut state = numbers.iter().map(|x|(*x as i64)*811589153).enumerate().collect::<Vec<_>>();

for _repeat in 0 .. 10 {
    for old_pos in 0 .. state.len() {
        // println!("{:3} {state:?}",old_pos.0); 
        let pos = state.iter().position(|p|p.0==old_pos).expect("Number vanished!");
        let value = state[pos].1;
        let new_pos = (pos as i64 + value).rem_euclid(numbers_len-1) as usize;
        if new_pos != pos {
            if new_pos < pos {
